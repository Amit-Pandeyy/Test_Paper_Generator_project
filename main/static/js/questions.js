    ...
    if (is_checked) {
        ...
        $.ajax({
            ...
            success: function (data) {
                    ...
                    ques_str +=
                        ...
                        <div class="flex-1 flex flex-col p-8">
                          ...
                          <dl class="mt-1 flex-grow flex flex-col justify-between">
                            <dd class="mt-3">
                              <span class="px-2 py-1 text-green-800 text-xs font-medium bg-green-100 rounded-full mb-4">${que.hardness}</span>
                            </dd>
                          </dl>
                            ${que.text}
                        </div>
                          ...
                          <div class="-mt-px flex divide-x divide-gray-200">
                            <div class="w-0 flex-1 flex">
                              <a class="relative -mr-px w-0 flex-1 inline-flex items-center justify-center py-4 text-sm text-gray-700 font-medium border border-transparent rounded-bl-lg hover:text-gray-500">
                                <button type="button" class="modal-question" pid="${item.pk}" data-modal-toggle="large-modal">See More</button>
                              </a>
                            </div>
                            ${!data.paper.includes(item.pk) ?
                            `<div class="-ml-px w-0 flex-1 flex">
                              <button class="relative w-0 flex-1 inline-flex items-center justify-center py-4 text-sm text-gray-700 font-medium border border-transparent rounded-br-lg hover:text-gray-500 add_to_cart"  pid="${item.pk}">
                                <span class="ml-3">Add to Paper</span>
                              </button>
                            </div>
                            ` :
                            `<div class="-ml-px w-0 flex-1 flex">
                                <button class="relative w-0 flex-1 inline-flex items-center justify-center py-4 text-sm text-gray-700 font-medium border border-transparent rounded-br-lg hover:text-gray-500 remove_from_cart"  pid="${item.pk}">
                                    <span class="ml-3">Click to Remove</span>
                                </button>
                          </div>
                ...
                <div id = 'topicDiv-${topic_id}' class="py-8">
                <h2 class="text-2xl font-semibold text-gray-900 mb-8">${topic_name}</h2>
...
$(document).on("click", '.remove_from_cart', function (event) {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.is_valid) {
                eml.innerHTML = `<div class="-ml-px w-0 flex-1 flex">
                <button class="relative w-0 flex-1 inline-flex items-center justify-center py-4 text-sm text-gray-700 font-medium border border-transparent rounded-br-lg hover:text-gray-500 add_to_cart"  pid="${id}">
                  <span class="ml-3">Add to Paper</span>
                </button>
              </div>`
...
$(document).on("click", '.add_to_cart', function (event) {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.is_valid) {
                eml.innerHTML = `<div class="-ml-px w-0 flex-1 flex">
                <button class="relative w-0 flex-1 inline-flex items-center justify-center py-4 text-sm text-gray-700 font-medium border border-transparent rounded-br-lg hover:text-gray-500 remove_from_cart"  pid="${id}">
                    <span class="ml-3">Click to Remove</span>
                </button>
          </div>`
                // x = document.getElementById('cart_items_count').innerText
